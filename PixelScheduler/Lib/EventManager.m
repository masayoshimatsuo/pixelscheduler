...
-(NSArray *)getEventsTodayCalendar
{
    ...
    NSDate *tomorrow = [[NSCalendar currentCalendar] dateByAddingComponents:tomorrowcomps toDate:toDay options:flags];
...
-(NSArray *)getEvents:(EKCalendar*)cal
{
    NSCalendar *curcal = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    
    NSUInteger flags = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay ;
    
    NSDateComponents *toDaycomps    = [curcal components:flags fromDate:[NSDate date]];
    NSDate *toDay = [curcal dateFromComponents:toDaycomps];
    ...
    NSPredicate *predicate = [self.eventStore predicateForEventsWithStartDate:toDay endDate:tomorrow calendars:@[cal]];
...
-(NSColor *)getSelectedCalendarColor{
